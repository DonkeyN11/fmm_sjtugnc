#!/usr/bin/env python3
"""
Synthetic data generator for the CMM command-line interface — ENHANCED VERSION.

Changes from original python/generate_data_cmm.py:
  - Extended MAX_SIGMA_PR from 10.0 to 30.0 for wider sigma range.
  - Added perpendicular road occlusion in satellite geometry
    (_generate_satellite_geometry_with_occlusion).
  - Added step fault injection on pseudorange (_inject_fault_into_pseudorange).
  - Added per-point ground truth edge_id (point_edge_ids) for segment-level accuracy.
  - New CLI params: --occlusion-angle, --occlusion-elevation-cutoff,
    --fault-probability, --fault-magnitude-min, --fault-magnitude-max.
  - TrajectoryConfig and TrajectoryResult dataclasses extended accordingly.
...
MIN_SIGMA_PR = 1.0
MAX_SIGMA_PR = 30.0  # Extended from 10.0 for wide-range sensitivity analysis
MAX_COVARIANCE_SAMPLING_ATTEMPTS = 64
MIN_TRAJECTORY_SPAN_M = 500.0  # Reduced for reliable path building on Hainan network
...
class Observation:
    ...
    edge_id: str = ""                    # Ground truth edge ID
    fde_detected: bool = False           # RAIM global test failed
    fde_excluded: int = 0                # Number of satellites excluded by FDE
    fde_sse: float = 0.0                 # SSE test statistic
    fde_threshold: float = 0.0           # Chi-square critical value
...
class TrajectoryResult:
    ...
    point_edge_ids: List[str] = None     # Per-point edge ID for segment-level accuracy

    def __post_init__(self):
        if self.point_edge_ids is None:
            self.point_edge_ids = []
...
def _load_roads(shapefile: Path) -> List[Tuple[List[List[float]], str, str, str]]:
    ...
    id_columns = [col for col in ("key", "fid", "id", "edge_id") if col in gdf.columns]
...
def _interpolate_along(coords: np.ndarray, target_dist_m: float,
                       cumulative: np.ndarray, seg_lengths: np.ndarray) -> Tuple[float, float, int]:
    """Interpolate a point at the specified cumulative distance (metres).

    Returns (x, y, seg_idx) where seg_idx is the segment index for edge lookup.
    """
    if target_dist_m <= 0:
        return float(coords[0, 0]), float(coords[0, 1]), 0
    if target_dist_m >= cumulative[-1]:
        return float(coords[-1, 0]), float(coords[-1, 1]), len(seg_lengths) - 1
    seg_idx = int(np.searchsorted(cumulative, target_dist_m, side="right") - 1)
    seg_idx = max(0, min(seg_idx, len(seg_lengths) - 1))
    seg_len = seg_lengths[seg_idx]
    if seg_len <= 0:
        return float(coords[seg_idx, 0]), float(coords[seg_idx, 1]), seg_idx
    ...
    x = x0 + fraction * (x1 - x0)
    y = y0 + fraction * (y1 - y0)
    return float(x), float(y), seg_idx
...
def _generate_satellite_geometry_with_occlusion(
    num_sats: int,
    ...
    """
    Generate satellite unit vectors, removing satellites in the cross-track
    occlusion zone (simulating buildings/trees along the roadside).

    The occlusion zone is a wedge centered on the direction perpendicular to
    the road heading (±90°). Satellites whose azimuth falls within
    ±occlusion_angle_deg of this perpendicular AND whose elevation is below
    occlusion_elevation_cutoff are rejected and re-sampled.

    Falls back to _generate_satellite_geometry when occlusion_angle_deg <= 0.
    """
    if occlusion_angle_deg <= 0.0:
        return _generate_satellite_geometry(num_sats, rng)

    perp_azimuth = math.radians((road_heading_deg + 90.0) % 360.0)
    mask_half_width = math.radians(occlusion_angle_deg)
    elev_cutoff = math.radians(occlusion_elevation_cutoff)

    def _in_occlusion_zone(az: float, el: float) -> bool:
        if el >= elev_cutoff:
            return False
        daz = abs(az - perp_azimuth)
        daz = min(daz, 2.0 * math.pi - daz)
        return daz <= mask_half_width

    vectors = []
    max_attempts = num_sats * 20
    attempts = 0
    while len(vectors) < num_sats and attempts < max_attempts:
        vec = _random_satellite_unit_vector(rng, min_el_deg=5.0)
        az = math.atan2(vec[1], vec[0])
        if az < 0:
            az += 2.0 * math.pi
        el = math.asin(max(-1.0, min(1.0, vec[2])))
        if not _in_occlusion_zone(az, el):
            vectors.append(vec)
        attempts += 1

    # Fallback: fill remaining with standard satellites if occlusion starves us
    while len(vectors) < num_sats:
        vectors.append(_random_satellite_unit_vector(rng))

    return np.stack(vectors[:num_sats], axis=0)


def _inject_fault_into_pseudorange(
    rho: np.ndarray,
    rng: np.random.Generator,
    ...
    """
    With probability fault_probability, inject a step fault on a random satellite.

    The fault adds a constant bias sampled from U(magnitude_min, magnitude_max)
    to ALL epochs of the selected satellite's pseudorange (step fault model).

    Returns:
        modified_rho: pseudorange array with fault applied (or unchanged)
        fault_metadata: dict with keys 'injected', 'sat_index', 'magnitude'
    """
    metadata = {"injected": False, "sat_index": -1, "magnitude": 0.0}
    if fault_probability <= 0.0 or rng.uniform() >= fault_probability:
        return rho, metadata

    sat_idx = int(rng.integers(0, rho.shape[0]))
    magnitude = rng.uniform(magnitude_min, magnitude_max)
    rho_modified = rho.copy()
    rho_modified[sat_idx] += magnitude

    metadata["injected"] = True
    metadata["sat_index"] = sat_idx
    metadata["magnitude"] = float(magnitude)
    return rho_modified, metadata
...
def _wls_position(
    ...
    **kwargs,  # fault_probability, fault_magnitude_min, fault_magnitude_max
    ...
    # Fault injection (step fault on a single random satellite)
    fault_meta = {"injected": False, "sat_index": -1, "magnitude": 0.0}
    if kwargs.get("fault_probability", 0.0) > 0.0:
        rho, fault_meta = _inject_fault_into_pseudorange(
            rho, rng,
            kwargs.get("fault_probability", 0.0),
            kwargs.get("fault_magnitude_min", 100.0),
            kwargs.get("fault_magnitude_max", 500.0),
        )

    # FDE parameters
    P_FA = 1e-5               # False alarm probability for global test
    MAX_EXCLUSIONS = 3          # Max satellites to exclude
    MIN_SATS_AFTER_EXCLUSION = 5

    excluded_indices: list = []
    fde_iterations = 0
    fde_ever_detected = False
    all_sat_indices = np.arange(num_sats)

    while True:
        active_mask = ~np.isin(all_sat_indices, excluded_indices)
        active_idx = all_sat_indices[active_mask]
        n_active = len(active_idx)

        if n_active < MIN_SATS_AFTER_EXCLUSION:
            break

        # --- WLS with active satellites ---
        sat_pos_active = sat_positions[active_idx]
        rho_active = rho[active_idx]
        ...
        for _ in range(max_iter):
            ranges = np.linalg.norm(sat_pos_active - x, axis=1)
            los = (sat_pos_active - x) / ranges[:, None]
            H = np.hstack((-los, np.ones((n_active, 1))))
            r = rho_active - ranges - b
            ...
            if np.linalg.norm(dx[:3]) < 1e-4:
                residuals = r - H @ dx  # final residuals
                break

        if last_H is None or Q is None:
            break

        n_state = H.shape[1]
        dof = n_active - n_state
        if dof <= 0:
            break

        # --- Global test (normalized SSE vs chi2) ---
        # Under H0: (r^T r) / sigma_pr^2 ~ chi2(dof)
        ...
        # chi2 critical value: P(chi2 > t_d) = P_FA
        # Using Wilson-Hilferty approximation for chi2 inverse
        from statistics import NormalDist
        z = NormalDist().inv_cdf(1.0 - P_FA)
        k = float(dof)
        t_d = k * (1.0 - 2.0/(9.0*k) + z * math.sqrt(2.0/(9.0*k))) ** 3

        fde_detected = normalized_SSE > t_d
        if fde_detected:
            fde_ever_detected = True

        if not fde_detected or fde_iterations >= MAX_EXCLUSIONS:
            break

        # --- Local test (w-test): identify worst satellite ---
        # S = I - H (H^T W H)^{-1} H^T W  — residual sensitivity matrix
        # With identity weight matrix W=I: S = I - H Q H^T
        S = np.eye(n_active) - last_H @ Q @ last_H.T
        w_scores = np.zeros(n_active)
        ...
        excluded_indices.append(worst_global_idx)
        fde_iterations += 1
    ...
    fde_info = {
        "fde_detected": fde_ever_detected,
        "fde_iterations": fde_iterations,
        "excluded_sats": excluded_indices,
        "global_test_SSE": float(normalized_SSE) if 'normalized_SSE' in locals() else 0.0,
        "global_test_threshold": float(t_d) if 't_d' in locals() else 0.0,
    }
    fault_meta.update(fde_info)

    return x, b, cov_xyz, last_H, fault_meta
...
def _generate_single(config: TrajectoryConfig) -> TrajectoryResult:
    ...
    # Generate base satellite geometry first (matching original rng order)
    _base_sats = _generate_satellite_geometry(config.num_sats, rng)
    ...
    for _ in range(max_global_attempts):
            ...
            while current_span < min_span_m and extension_counter < max_extension_steps:
                ...
                if new_edge_id is not None and mode:
                    if mode == "append":
                        ...
                        path_point_edge.extend([new_edge_id] * n_new_pts)
                    elif mode == "prepend":
                        edge_sequence.insert(0, new_edge_id)
                        path_point_edge[0:0] = [new_edge_id] * n_new_pts
        ...
        # Build segment-to-edge mapping: segment i (between points i,i+1)
        # belongs to the edge of the starting point of the segment.
        seg_start_to_edge_data = path_point_edge[:len(chosen_coords) - 1]

        # Compute road heading for occlusion
        road_heading_deg = 0.0
        if config.occlusion_angle_deg > 0.0 and chosen_coords.shape[0] >= 2:
            dx = float(chosen_coords[-1, 0] - chosen_coords[0, 0])
            dy = float(chosen_coords[-1, 1] - chosen_coords[0, 1])
            road_heading_deg = math.degrees(math.atan2(dy, dx))
            if road_heading_deg < 0:
                road_heading_deg += 360.0

        # Satellite geometry: use occlusion-aware version if enabled, else base
        if config.occlusion_angle_deg > 0.0:
            sats_unit = _generate_satellite_geometry_with_occlusion(
                config.num_sats, rng,
                road_heading_deg=road_heading_deg,
                occlusion_angle_deg=config.occlusion_angle_deg,
                occlusion_elevation_cutoff=config.occlusion_elevation_cutoff,
            )
        else:
            sats_unit = _base_sats  # reuse pre-generated base geometry
        ...
        for seq, dist_m in enumerate(sample_distances):
            ...
            # Record per-point edge ID for segment-level accuracy
            point_edge_id = seg_start_to_edge_data[seg_idx] if seg_idx < len(seg_start_to_edge_data) else chosen_edges[-1]
            point_edge_ids.append(point_edge_id)
            ...
            observations.append(
                Observation(
                    ...
                    edge_id=point_edge_id,
                    fde_detected=fault_meta.get("fde_detected", False),
                    fde_excluded=len(fault_meta.get("excluded_sats", [])),
                    fde_sse=fault_meta.get("global_test_SSE", 0.0),
                    fde_threshold=fault_meta.get("global_test_threshold", 0.0),
                )
            )
    ...
    for offset in range(count):
        ...
        tasks.append(
            TrajectoryConfig(
                ...
                num_sats=num_sats,
                occlusion_angle_deg=occlusion_angle_deg,
                occlusion_elevation_cutoff=occlusion_elevation_cutoff,
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--occlusion-angle", type=float, default=0.0,
                        help="Half-width of perpendicular occlusion wedge (deg). 0=no occlusion.")
    parser.add_argument("--occlusion-elevation-cutoff", type=float, default=30.0,
                        help="Elevation cutoff for occlusion zone (deg).")
    parser.add_argument("--fault-probability", type=float, default=0.0,
                        help="Probability of injecting a step fault per trajectory.")
    parser.add_argument("--fault-magnitude-min", type=float, default=100.0,
...
def _write_observation_csv(results: List[TrajectoryResult], destination: Path) -> None:
    """Write observations in CMM point-mode format (WGS84 lon/lat degrees).

    Matches dataset-hainan-06/cmm_input_points.csv format:
      id;timestamp;x;y;sde;sdn;sdu;sdne;sdeu;sdun;protection_level
    with additional FDE columns appended.
    """
    import pyproj
    utm_epsg = 32649  # UTM zone 49N
    ...
    with destination.open("w", encoding="utf-8", newline="") as fh:
        ...
        for traj in sorted(results, key=lambda item: item.traj_id):
            for obs in traj.observations:
                # Reproject UTM → WGS84
                lon, lat = transformer.transform(obs.x, obs.y)
                cos_lat = math.cos(math.radians(lat))
                jx = 1.0 / (METERS_PER_DEG_LAT * cos_lat)  # meter→degree at this lat
                jy = 1.0 / METERS_PER_DEG_LAT

                # Transform covariance from meters to degrees via Jacobian
                sde_deg = abs(jx) * obs.sde
                sdn_deg = abs(jy) * obs.sdn
                sdu_deg = abs(jy) * obs.sdu  # rough approximation for vertical
                sdne_deg = jx * jy * obs.sdne
                sdeu_deg = jx * jy * obs.sdeu
                sdun_deg = jy * jy * obs.sdun

                # Transform protection level meters → degrees
                pl_deg = obs.protection_level * max(abs(jx), abs(jy))
                ...
                writer.writerow(
                    [
                        ...
                        f"{sdun_deg:.16f}",
                        f"{pl_deg:.12f}",
                        str(int(obs.fde_detected)),
                        str(obs.fde_excluded),
                        f"{obs.fde_sse:.6f}",
                        f"{obs.fde_threshold:.6f}",
                    ]
                )
...
def _write_ground_truth_csv(results: List[TrajectoryResult], destination: Path) -> None:
    header = ["id", "geom", "timestamp", "edge_ids", "point_edge_ids"]
    destination.parent.mkdir(parents=True, exist_ok=True)
    with destination.open("w", encoding="utf-8", newline="") as fh:
        writer = csv.writer(fh, delimiter=";")
        writer.writerow(header)
        for traj in sorted(results, key=lambda item: item.traj_id):
            ...
            point_edge_str = json.dumps(traj.point_edge_ids, ensure_ascii=False)
            writer.writerow([traj.traj_id, traj.truth_wkt, timestamp_str, edge_str, point_edge_str])
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    tasks = _make_tasks(
        ...
        args.max_sigma_pr,
        args.num_sats,
        occlusion_angle_deg=args.occlusion_angle,
        occlusion_elevation_cutoff=args.occlusion_elevation_cutoff,
