#!/usr/bin/env python3
"""Candidate count comparison: CaMM vs HMM."""
...
ax.bar(x-0.175, means, 0.35, color=C_CaMM, label="CaMM (HPL-adaptive)", edgecolor="white",lw=0.5)
ax.bar(x+0.175, [16]*len(labels), 0.35, color=C_HMM, label="HMM (fixed r=0.03, k=16)", edgecolor="white",lw=0.5)
for i,v in enumerate(means): ax.text(i-0.175, v+0.3, f"{v:.1f}", ha="center", fontsize=8, fontweight="bold", color=C_CaMM)
ax.set_xticks(x); ax.set_xticklabels(labels)
ax.set_ylabel("Mean candidates per epoch")
ax.set_title("Candidate Count: CaMM (HPL-adaptive) vs HMM (fixed-radius)")
...
if cmm_file.exists():
    ...
    ax.bar(0, tavg, 0.45, color=C_CaMM, label="CaMM (HPL-adaptive)", edgecolor="white",lw=0.5)
    ax.bar(1, 16, 0.45, color=C_HMM, label="HMM (fixed r=0.03, k=16)", edgecolor="white",lw=0.5)
    ax.text(0, tavg+0.3, f"{tavg:.1f}", ha="center", fontsize=10, fontweight="bold", color=C_CaMM)
    ax.text(1, 16.3, "16", ha="center", fontsize=10, fontweight="bold", color=C_HMM)
    ax.set_xticks([0,1]); ax.set_xticklabels(["CaMM","HMM"])
    ax.set_ylabel("Mean candidates per epoch")
