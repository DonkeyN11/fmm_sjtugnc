...
void FMM::CONFIG::ResultConfig::print() const {
  ...
  if (output_config.write_h0_lambda)
    ss << "h0_lambda ";
...
FMM::CONFIG::ResultConfig FMM::CONFIG::ResultConfig::load_from_xml(
  ...
  if (xml_data.get_child_optional("config.output.fields")) {
    ...
    for (auto& v : fields) {
      ...
      } else if (v.first == "h0_lambda") {
        config.output_config.write_h0_lambda = true;
...
FMM::CONFIG::ResultConfig FMM::CONFIG::ResultConfig::load_from_arg(
  ...
  if (arg_data.count("output_fields") > 0) {
    ...
    if (dict.find("h0_lambda") != dict.end()) {
      config.output_config.write_h0_lambda = true;
    }
...
void FMM::CONFIG::ResultConfig::register_help(std::ostringstream &oss){
  ...
  oss<<"  offset,error,spdist,sp_dist,eu_dist,tp,ep,trustworthiness,n_best_trustworthiness,candidates,cumu_prob,length,duration,speed,timestamp,delta_entropy,posterior_entropy,h0_lambda,seq,point_mode,all\n";
};
