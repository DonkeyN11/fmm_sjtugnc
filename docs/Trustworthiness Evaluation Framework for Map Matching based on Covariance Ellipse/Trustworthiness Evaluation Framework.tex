...
This section aims to verify that the proposed emission and transition probability models are accurate under controlled nominal conditions, demonstrate the advantages of CMM over FMM in both accuracy and availability, and illustrate the mechanism by which the proposed trustworthiness metric varies with road network complexity and observation bias.

Monte Carlo experiments are conducted under the condition of pseudorange standard deviation. By adding Gaussian random noise to the distance between a synthetic constellation and ground truth, pseudorange observations are generated and the vehicle position is obtained via the weighted least-squares algorithm. This simulation aims at three aspects:
...
Ground-truth trajectories are generated by randomly selecting connected road segments from the Haikou, Hainan road network. Synthetic constellations are constructed to simulate pseudorange observations by adding Gaussian noise with a given standard deviation, as modeled in Eq.~\eqref{eq:rho_model}.

The pseudorange noise standard deviation ranges from 1\,m to 10\,m for nominal conditions and 10\,m to 100\,m for challenging environments. Trajectories are simulated with a constant 12\,m/s velocity for straight motion and linear velocity change in both magnitude and direction for maneuvers. Each trajectory contains 1000 sample points, and every 10 trajectories share the same $\sigma$ value. The setup is summarized in Table~\ref{tab:mc_setup}.
...
observation model, following the linearized GNSS positioning model of
Eq.~\eqref{eq:lin_model}, is
\[
...
Section~\ref{sec:proposed} and enables direct control of positioning uncertainty
in the simulation.

\subsubsection{Noise Regimes and Anisotropy}
To emulate environment-dependent GNSS performance ranging from open-sky to urban
canyon conditions, the standard deviations $\sigma_x$ and $\sigma_y$ and the
correlation coefficient $\rho$ are sampled from uniform distributions within
regime-dependent ranges, with $\sigma$ ranging from $1$\,m (nominal) to
$100$\,m (degraded). The noise statistics are modeled as \emph{piecewise constant}
over short environment segments ($30$--$60$\,s) to reflect slowly varying
propagation conditions. The covariance matrix is constructed according to
Eq.~\eqref{eq:cov_emission}. This parameterization yields error ellipses with
varying magnitude and eccentricity, capturing both near-isotropic open-sky errors
and highly anisotropic urban errors.
...
road segments. The emission probability uses an isotropic Gaussian distribution
with a fixed variance as shown in Eq.~\eqref{eq:classic_emission}. No GNSS
covariance, protection level, or integrity-related information is used in
candidate generation or likelihood modeling.

\subsubsection{Proposed Covariance-Aware HMM Method}
The proposed method, referred to as \textbf{Covariance Map Matching (CMM)},
incorporates GNSS uncertainty information consistently throughout the HMM
pipeline. Candidate road segments are selected using an adaptive search region
defined by the horizontal protection level (HPL). Candidate points are generated
via Mahalanobis projection, ensuring statistical consistency with the anisotropic
GNSS covariance. The emission probability is modeled as a covariance-aware
bivariate Gaussian (Eq.~\eqref{eq:cov_emission}), and the fixed-lag smoothing
posterior serves as a per-epoch trustworthiness statistic
(Eq.~\eqref{eq:trustworthiness}).

\subsubsection{Ablation Configurations}
To isolate the contribution of individual components, several ablation variants
are evaluated:
\begin{itemize}
    \item \textbf{Anisotropic Emission}: The fixed isotropic emission model is
    replaced by the covariance-based Gaussian emission probability, while
    candidate search and projection remain identical to the baseline.
    \item \textbf{Mahalanobis Projection}: Euclidean projection is replaced by
    ...
    replaced by an adaptive search region defined by the HPL, while Euclidean
    projection and isotropic emission are retained.
    \item \textbf{Full CMM}: All components of the proposed method are enabled,
    including adaptive candidate search, Mahalanobis projection, covariance-based
    emission modeling, and fixed-lag smoothing trustworthiness evaluation.
\end{itemize}
