...
This section situates the present work at the intersection of three
...
Since the seminal work of Newson and Krumm~\cite{HMM}, Hidden Markov
...
standard formulation treats each GNSS fix as an observation and
candidate road positions as hidden states, using the Viterbi algorithm
...
Several lines of research have extended the basic HMM framework.
...
problem inherent in local normalization of HMM. Fu \emph{et
al.}~\cite{Fu2021} and Guo \emph{et al.}~\cite{Guo2025_HMM}
...
a Variable Sliding Window (VSW) method that guarantees online solution
...
patterns through kernel density estimation. Huang \emph{et
al.}~\cite{Huang2024} addressed drifting, ping-pong, and U-turn errors
in mobile phone signaling data using an incremental HMM algorithm,
achieving 82.9\% precision and 93.6\% recall on large-scale urban
datasets. At the lane-level frontier, Hansson \emph{et
...
matching literature: \emph{all of the above methods assume an isotropic
...
($\Sigma_t$) that modern receivers output through NMEA GST messages.
The present work addresses this gap by replacing the scalar emission
model with a full-covariance Mahalanobis formulation.
...
(RAIM)~\cite{Zhang2018} detects faulty satellites through
residual-based consistency checks, requiring at least five visible
satellites for fault detection and six for fault exclusion. Advanced
RAIM (ARAIM)~\cite{Zhang2019} extends this framework to
...
computation, and Robust Modeling—and notes that 73.3\% of published
integrity research remains GNSS-centric.

The extension of GNSS integrity to automotive and urban environments has
attracted growing attention. Maaref and Kassas~\cite{Maaref2022}
...
However, none of these works integrate GNSS integrity concepts—PL
computation, PHMI risk bounds, and the full covariance
matrix—\emph{within} the HMM map matching emission and candidate search
framework. The present work bridges this gap by using RAIM-derived
HPL as a geometry-constrained candidate search radius and the PHMI
integrity risk as an emission probability weighting factor, making map
matching explicitly consistent with GNSS integrity theory.
...
segments) is extensively studied, the \emph{calibration} of output
probabilities—whether confidence scores reflect true correctness
rates—has received little attention in the map matching literature.
In machine learning, Expected Calibration Error (ECE)~\cite{Guo2017}
and reliability diagrams are standard tools for evaluating
probabilistic classifier calibration. Nixon \emph{et
...
\emph{et al.}~\cite{Quddus2006}, who proposed an empirical integrity
...
GPS/DR. Kempinska \emph{et al.}~\cite{Kempinska2017} developed
Probabilistic ST-Matching (PST-Matching), which outputs confidence
...
probabilities. Wang \emph{et al.}~\cite{Wang2023_PMHT} proposed a
Probabilistic Multiple-Hypotheses Tracker (PMHT-MM) that weights
...
information density for confidence scoring. Phillips \emph{et
...
ECE by model component (emission, transition, smoothing).

The present work brings together calibration theory from machine
...
softmax-normalized fixed-lag smoothing posterior) for every matched
epoch, evaluate its calibration through reliability diagrams and ECE,
and decompose the calibration improvement via ablation studies to
quantify the per-component contribution of anisotropic Mahalanobis
emission, fixed-lag smoothing, and PHMI integrity constraints. To our
