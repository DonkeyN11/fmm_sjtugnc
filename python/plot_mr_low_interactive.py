...
"""
交互式低概率轨迹可视化脚本
使用Plotly创建可交互的轨迹地图，专门用于mr_low.csv数据
...
def create_interactive_mr_low_visualization(csv_file_path, output_html_path=None, max_trajectories=2000):
    """创建mr_low数据的交互式轨迹可视化"""
    print(f"开始处理mr_low数据: {csv_file_path}")

    # 读取CSV文件
    try:
        df = pd.read_csv(csv_file_path, sep=';')
        print(f"成功读取 {len(df)} 条低概率轨迹数据")
    except Exception as e:
        print(f"读取CSV文件失败: {e}")
        return False
    ...
    for idx, row in df.iterrows():
        ...
        if coords and len(coords) >= 2 and last_cumu_prob is not None:
            ...
            normalized_prob = min(abs(last_cumu_prob) / 100, 1.0)  # 归一化到0-1
            red_component = int(255 * normalized_prob)
            blue_component = int(255 * (1 - normalized_prob))
            ...
            fig.add_trace(go.Scatter(
                ...
                name=f'Low Prob Trajectory {trajectory_id}',
    ...
    fig.update_layout(
        title={
            'text': 'Low Cumulative Probability Trajectories (10% Bottom)',
            'x': 0.5,
            'xanchor': 'center',
    ...
    # 创建输出目录
    os.makedirs('output/lowprob', exist_ok=True)
    ...
    if output_html_path:
        ...
        # 默认保存路径
        default_path = 'output/lowprob/mr_low_interactive_visualization.html'
...
def main():
    """主函数"""
    # 设置输入文件路径（使用mr_low.csv数据）
    csv_file_path = 'python/output/filtered/mr_low_filtered.csv'

    # 检查文件是否存在
    if not os.path.exists(csv_file_path):
        print(f"错误: 文件 {csv_file_path} 不存在")
        print("请先运行 filter_low_cumu_prob.py 来生成mr_low.csv")
        return
    ...
    # 可选：调整显示的轨迹数量
    max_trajectories = 500  # 减少轨迹数量以提高性能

    # 执行可视化
    success = create_interactive_mr_low_visualization(csv_file_path, output_html_path, max_trajectories)
